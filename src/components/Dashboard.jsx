...
import { Award, Target, BookOpen, Briefcase, ChevronRight, Zap } from 'lucide-react';
...
export default function Dashboard({ 
  ...
  const completedGoalsCount = careerData.weeklyGoals.filter(g => g.completed).length;
  const totalGoalsCount = careerData.weeklyGoals.length;
  const goalsPercentage = totalGoalsCount > 0 ? (completedGoalsCount / totalGoalsCount) * 100 : 0;

  // Next recommended roadmap step
  const nextRoadmapStep = careerData.roadmap.find(step => step.status === 'active') || 
          ...
          <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '1rem' }}>
              <p style={{ fontStyle: 'italic', fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                "{careerData.advisorGreeting.slice(0, 110)}..."
              </p>
            ...
            <div className="goals-list" style={{ gap: '0.5rem' }}>
              {careerData.weeklyGoals.map(goal => (
                <div 
                  ...
                  style={{ padding: '0.6rem 0.8rem', pointerEvents: 'none' }} // Readonly preview on dashboard
                >
